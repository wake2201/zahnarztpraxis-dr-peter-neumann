...
import type { UserAccount, UserAccountRole } from "./types";
...
export function UsersTab({ users }: Props) {
  ...
  const [optimisticUsers, addOptimisticDeleteUser] = useOptimistic(
    users,
    (state: UserAccount[], userIdToDelete: string) => state.filter((user) => user.id !== userIdToDelete),
  );
        ...
        <form onSubmit={handleCreateUser} className="grid sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <Input
              ...
              placeholder="mitarbeiter@praxis.de"
            />
          </div>
          <div>
            ...
            <div className="relative">
              ...
              <button
                ...
                title={showNewUserPassword ? "Passwort verbergen" : "Passwort anzeigen"}
              >
      ...
      <div className="bg-white rounded-2xl shadow-card border border-slate-100 overflow-hidden">
        ...
        <div className="divide-y divide-slate-100">
          {optimisticUsers.map((user) => (
            <div key={user.id} className="px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${roleBadgeClass(user.role)}`}>
                  {user.role === "admin" ? <Shield className="w-4 h-4 text-primary" /> : <Users className="w-4 h-4 text-slate-500" />}
                </div>
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-slate-800 truncate">{user.name || "Unbenannt"}</p>
                  <p className="text-xs text-slate-500 truncate">{user.email} · {roleLabel(user.role)}</p>
                </div>
              </div>
              {user.role === "staff" && (
...
function roleLabel(role: UserAccountRole) {
  switch (role) {
    case "admin":
      return "Admin";
    case "staff":
...
function roleBadgeClass(role: UserAccountRole) {
  if (role === "admin") {
